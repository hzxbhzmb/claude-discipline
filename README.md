...
│   ├── test-tribunal.js         # stub-mode self-test（不烧 token）
│   └── test-tribunal-real.js    # real-mode 端到端集成测试（会真烧 API）
├── rules/
...
```bash
# Stub mode（不烧 token，秒级返回）
node scripts/test-tribunal.js

# Real mode（真 spawn claude -p，慢且烧 token，用 haiku 省钱）
node scripts/test-tribunal-real.js
```
